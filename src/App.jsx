...
export default function App() {
  ...
  const [listaDocumentos, setListaDocumentos] = useState(() => {
    const datosGuardados = localStorage.getItem('documentos_genova');
    return datosGuardados ? JSON.parse(datosGuardados) : [];
  }); //acá lo que hacemos es crear el estado de listaDocumentos y el modificador setListaDocumentos que va a ir cambiando el estado de listaDocumetnos conforme se haga ejecutar.

  useEffect(() => {
    localStorage.setItem('documentos_genova', JSON.stringify(listaDocumentos));
  }, [listaDocumentos]);
